...
const SignInScreen = ({ navigation }) => {
  ...
  useEffect(() => {
    if (isError) {
      Alert.alert('Lỗi đăng nhập', message || 'Tài khoản hoặc mật khẩu không chính xác');
      dispatch(reset());
    }
    if (isSuccess || user) {
      dispatch(reset());
  ...
  return (
    <ScrollView contentContainerStyle={styles.container} bounces={false}>
      {/* Top Banner Image */}
      <ImageBackground
        source={{ uri: 'https://images.unsplash.com/photo-1578985545062-69928b1d9587?w=800' }}
      ...
      <View style={styles.formContainer}>
        ...
        <Text style={styles.subtitle}>Sign in to your sweet recipe account</Text>
